...
var App = new Class({
  initialize: function() {
    ...
    options = {fixed: true, item_classes: 'hcentered', vcentered: true};

    var items = [{text: 'loading...'}];
    this.loadingPage = Pages.createSplashMenu(null, 'loading', items, options);

    var items = [{text: 'create drawing...'}];
    this.createDrawingPage = Pages.createSplashMenu(null, 'createDrawing', items, options);

    var items = [{text: 'solved'}];
    this.solvedPage = Pages.createSplashMenu(null, 'solved', items, options, {obj:this, method:'menuSelection'});

    var items = [{text: 'shuffling...'}];
    this.shufflingPage = Pages.createSplashMenu(null, 'shuffling', items, options);
    ...
    this.selectionPage = Pages.createSplashMenu(null, 'selection', items, options, {obj:this, method:'menuSelection'});
  ...
  menuSelection: function(type, menu) {
    var text = menu.items[menu.selectedId].text;
    if ('shuffle' === text) {
      Pages.showPage(this.shufflingPage);
      this.shuffle(11);
    } else if ('new image' === text) {
      this.loadNextImage();
    } else if ('new graphic' === text) {
      this.doCreateDrawing();
    } else if ('cancel' === text) {
      Pages.hidePage();
    } else if ('solved' === text) {
      Pages.showPage(this.selectionPage);
    }
  },
  ...
  loadImage: function(idx) {
    Pages.showPage(this.loadingPage);
    if (!this.images) {
      this.images = [];
    }
    this.imageIdx = idx;
  ...
  doCreateDrawing: function(f) {
    Pages.showPage(this.createDrawingPage);
  ...
  enable: function(val) {
    ...
    if (val) {
      ...
      // show semi transparent canvas -> must be done with Pages.showPage
      Pages.hidePage();
    }
  },
  ...
  solved: function() {
    ...
    this.draw();
    Pages.showPage(this.solvedPage);
  },
  onResize: function() {
    this.resize();
    this.draw();
...
old version, using deprecated simple menu
